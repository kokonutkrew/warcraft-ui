...
function fn:msgMod(msg, name, noErr)
	...
	if msg:find("KK_LINK") then
		local club, link
		kkClubId = 382392881
		--club = ClubFinderGetCurrentClubListingInfo(kkClubId)
		--if club then
		--	link = GetClubFinderLink(club.clubFinderGUID, club.name)
		--	DB.global.guildLinks[link:match("%[.*: (.*)%]")] = link
		--end

		--link = GetCommunityLink(kkClubId)
		--DB.global.guildLinks[link:match("%[.*: (.*)%]")] = link

		--DB.global.guildLinks = DB.global.guildLinks or {}

		--DB.global.community = {}
		--if DB.global.community[GetGuildInfo("player")] then
		--	link = DB.global.community[GetGuildInfo("player")]
		--else
		--	link = GetCommunityLink(kkClubId)
		--	DB.global.community[link:match("%[.*: (.*)%]")] = link
		--end
		...
		msg = msg:gsub("KK_LINK", link and link:gsub(" ", " ") or 'K_LINK')
