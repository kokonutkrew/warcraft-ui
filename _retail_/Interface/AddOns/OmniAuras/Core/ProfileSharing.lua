...
function PS:ShowProfileDialog(text) -- XXX don't share dialog w/ OmniCD
	if not Dialog then
		Dialog = CreateFrame("Frame", "OmniAuras_ProfileDialog", UIParent, "DialogBoxFrame") -- inherit to draw over AceConfig, other methods didn't work unless strata is set to tooltip.
...
--		Dialog:SetUserPlaced(false) -- clamped and nonamed frames aren't saved to local file
		...
		if Dialog.SetResizeBounds then -- DF
		...
		-- Button
		_G.OmniAuras_ProfileDialogButton:Hide()

		local CloseButton = E.Libs.OmniCDC.CreateFlashButton(Dialog, CLOSE) -- inherits DialogBoxFrame Button
		...
		DecodeButton:SetScript("OnClick", function()
			...
			if profileType then
				profileKey = (profileType == "blacklist" or profileType == "cds") and "" or format("%s: |cffffd200%s|r", L["Profile"], profileKey)
		...
		-- Resizer
		local Resizer = CreateFrame("Button", "OmniAuras_ProfileDialogResizeButton", Dialog)
...
--			self:GetHighlightTexture():Show()
		...
		-- ScrollContainer
		local ScrollContainer = CreateFrame("Frame", "OmniAuras_ProfileDialogScrollContainer", Dialog, BackdropTemplateMixin and "BackdropTemplate" or nil)
		...
		-- ScrollFrame
		local ScrollFrame = CreateFrame("ScrollFrame", "OmniAuras_ProfileDialogScrollFrame", ScrollContainer, "UIPanelScrollFrameTemplate")
		...
		-- ScrollBar
		local ScrollBar = _G["OmniAuras_ProfileDialogScrollFrameScrollBar"]
		ScrollBar:ClearAllPoints()
		ScrollBar:SetPoint("TOPLEFT", ScrollContainer, "TOPRIGHT", 4, -1)
		ScrollBar:SetPoint("BOTTOMLEFT", ScrollContainer, "BOTTOMRIGHT", 4, 1)
		-- Scroll button visibility updates on OnValueChanged. (AceGUI, we can just hide)
--		ScrollBar.ScrollUpButton:Hide()
--		ScrollBar.ScrollDownButton:Hide()
		ScrollBar.ScrollUpButton:SetNormalTexture(0) -- DF: 0
		...
		-- EditBox
		local EditBox = CreateFrame("EditBox", "OmniAuras_ProfileDialogEditBox", ScrollFrame)
...
function PS:CopyBlacklistAuras(profileData)
	for k, v in pairs(profileData) do
		OmniAurasDB.global.auraBlacklist[k] = v
		E:AddAuraToBlacklist(k)
	end
end

function PS:CopyCustomSpells(profileData)
	for k, v in pairs(profileData) do
		OmniAurasDB.cooldowns[k] = v
	end
end
...
function PS:ImportProfile(encodedData)
	...
	while true do
		...
		if not OmniAurasDB.profiles[key] then
			profileKey = key
			break
		end
		n = n + 1
	...
	if profileType == "blacklist" then
		E.Libs.OmniCDC.StaticPopup_Show("OMNIAURAS_IMPORT_BLACKLIST", nil, nil, profileData)
	elseif profileType == "cds" then
		E.Libs.OmniCDC.StaticPopup_Show("OMNIAURAS_IMPORT_EDITOR", nil, nil, profileData)
	else
		E.Libs.OmniCDC.StaticPopup_Show("OMNIAURAS_IMPORT_PROFILE", format("|cffffd200%s|r", profileKey), nil, {profileType=profileType, profileKey=profileKey, profileData=profileData})
	end

	return profileType, profileKey
end

-- OmniAurasDB keys: global, profileKeys, profile, version, cooldowns
local blackList = {
	modules = true,
}
...
function PS:ExportProfile(profileType)
	...
	local profileData
	if profileType == "blacklist" then
		profileData = E:DeepCopy(OmniAurasDB.global.auraBlacklist)
	elseif profileType == "cds" then
		profileData = E:DeepCopy(OmniAurasDB.cooldowns)
	else
		profileData = E:DeepCopy(OmniAurasDB.profiles[profileKey], blackList)
	...
	serializedData = format("%sOmniAuras|%s|%s", serializedData, profileType, profileKey)
