...
local FFSentHealthWarning = false; --Flag to prevent spamming of health warning messages
    
function FF_OnEvent(self, event, ...) --... refers to the arguments which may be passed with the event

    if (event == "ADDON_LOADED") then--If an addon was just loaded
    
        local arg1 = ...; --Load the arguments passed with the event
    ...
    if (FF_Options["Enabled"] == 1) then
        ...
        if (event == "CHAT_MSG_WHISPER") then --If the event was a whisper
            local arg1, arg2 = ...; --Load the arguments passed with the event

            if(UnitIsSameServer(arg2, "player")) then -- Sender is on my server
                FF_Sender = string.sub(arg2, 1, strfind(arg2, "-")-1); -- truncate name to just player, not including servername
            else
                FF_Sender = arg2; -- use full name
            end

            --FFDebug("got event CHAT_MSG_WHISPER from "..arg2.." who is really "..FF_Sender);

            FF_ParseWhisper(arg1, FF_Sender);
        end
        
        if (event == "CHAT_MSG_ADDON") then --If the event was a message from an addon
            local arg1, arg2, arg3, arg4 = ...; --Load the arguments passed with the event
            FF_ParseAddonMessage(event, arg1, arg2, arg3, arg4);
        end
        ...
        if (event == "AUTOFOLLOW_BEGIN") then
            ...
            if (FF_Announce_Following == true) then
                ...
                if (FF_Options_Announcements["AnnounceFollowStart"] == 1) then
                    ...
                    elseif (GetNumSubgroupMembers() > 0) then --If player is in a party
                        FF_SendPartyMessage(FFMSG_FOLLOWING..FF_Leader); --Tell the party who we are following
                    end
                    
                end --End: if (FF_Options_Announcements["AnnounceFollowStart"] == 1) then
                
            end --End: if (FF_Announce_Following == true) then
        ...
        if (event == "AUTOFOLLOW_END") then
            ...
            if (FF_Leader ~= "") then --Check if FF_Leader is empty (see comment above for details)
                ...
                if (FF_Announce_Following == true) then
                    ...
                    end --End: if (FF_Options_Announcements["AnnounceFollowStop"] == 1) then
                    
                end --End: if (FF_Announce_Following == true) then
                
                SendAddonMessage(FF_AddonMessagePrefix, "StoppedFollowingYou", "WHISPER", FF_Leader); --Send a message to the person I'm following (If they are using Follow Felankor they will be told I have stopped following)
                
            end --End: if (FF_Leader ~= "") then
            
            FF_Announce_Following = false;
            FF_LastFollowed = FF_Leader; --Store the name of the last person that was followed
            FF_Leader = ""; --Clear the leader
            
        end --End: if (event == "AUTOFOLLOW_END") then
        
        if (event == "PLAYER_REGEN_ENABLED") then --When combat ends
            
            if (FF_Options_DualBox["ResumeFollow"]["Enabled"] == 1) then --If the Auto Resume Follow option is enabled
                
                if (FF_Following == false) then --If I am not already following
                    
                    FF_LastFollowedID = FF_AuthoriseSender(FF_LastFollowed, FF_Options_DualBox["ResumeFollow"]["Group"], FF_Options_DualBox["ResumeFollow"]["Raid"], FF_Options_DualBox["ResumeFollow"]["Guild"], FF_Options_DualBox["ResumeFollow"]["Friends"], FF_Options_DualBox["ResumeFollow"]["Master"]);--Check if the player should have resume follow
                    
                    if (FF_LastFollowedID ~= nil) then --If we should resume follow
                        
                        FF_ParseWhisper(FFWHISP_FOLLOW, FF_LastFollowed); --Simulate a follow me whisper from the DualBox master (i.e. start following again)
                        
                    end
                    
                end --End: if (FF_Following == false) then
                
            end --End: if (FF_Options_DualBox["ResumeFollow"] == 1) then
            
        end -- End: if (event == "PLAYER_REGEN_ENABLED") then
        
        if (event == "UNIT_SPELLCAST_SUCCEEDED") then --When the player finishes casting a spell
            
            local arg1 = ...; --Get the name of the person who cast the spell
            
            if (FF_Options_DualBox["CancelResumeOnSpellCast"] == 1) then --If Auto Resume Follow should be canceled when a spell is cast
                
                if (arg1 == "player") then --If it was the player who cast the spell and not a group/raid member
                    
                    if (string.len(FF_LastFollowed) > 0) then
                        FF_CancelAutoResumeFollow();
                    end
                    
                end --End: if (arg1 == "player") then
                
            end -- End: if (FF_Options_DualBox["CancelResumeOnSpellCast"] == 1) then
            
        end --End: if (event == "UNIT_SPELLCAST_SUCCEEDED") then
        ...
        if (event == "PARTY_INVITE_REQUEST") then
            ...
            --FFDebug("party_invite_request from ("..arg1..")");
            
            --FFDebug("AllowAutoAcceptParty[Enabled] = "..FF_Options["AllowAutoAcceptParty"]["Enabled"]);
              
            if (FF_Options["AllowAutoAcceptParty"]["Enabled"] == 1) then
                --FF_Sender=arg1; --Get name of sender
              if(UnitIsSameServer(arg1, "player")) then -- Sender is on my server
                FF_Sender = string.sub(arg1, 1, strfind(arg1, "-")-1); -- truncate name to just player, not including servername
              else
                FF_Sender = arg1; -- use full name
              end
              
              --FFDebug("party_invite_request from "..FF_Sender.." ("..arg1..")");
              
                FF_SenderID = FF_AuthoriseSender(FF_Sender, false, false, FF_Options["AllowAutoAcceptParty"]["Guild"], FF_Options["AllowAutoAcceptParty"]["Friends"], FF_Options["AllowAutoAcceptParty"]["Master"]);--Check if the player is a guild member or a friend
        ...
        if (event == "GROUP_ROSTER_UPDATE") then
            ...
            if (GetNumSubgroupMembers() > 0) then --If the player is in a group
        ...
        if (event == "RESURRECT_REQUEST") then
            ...
            local arg1 = ...; --Get the name of the sender
            if(UnitIsSameServer(arg1, "player")) then -- Sender is on my server
                FF_Sender = string.sub(arg1, 1, strfind(arg1, "-")-1); -- truncate name to just player, not including servername
            else
                FF_Sender = arg1; -- use full name
            ...
            if (FF_Options["AllowAcceptResurrect"]["Enabled"] == 1) then
                
                FF_SenderID = FF_AuthoriseSender(FF_Sender, FF_Options["AllowAcceptResurrect"]["Group"], FF_Options["AllowAcceptResurrect"]["Raid"], FF_Options["AllowAcceptResurrect"]["Guild"], FF_Options["AllowAcceptResurrect"]["Friends"], FF_Options["AllowAcceptResurrect"]["Master"]);--Check if the player is a guild, raid or party member or a friend
                if (FF_SenderID ~= nil) then --If the sender was authorised
                    
                    AcceptResurrect(); --Does exactly what it says on the tin! :)
                    ...
                    if (FF_Options_Announcements["AnnounceResurrection"] == 1) then
                        ...
                        elseif (GetNumSubgroupMembers() > 0) then
                            ...
                            FF_SendWhisper(FFMSG_RESURRECT_AUTO_ACCEPTED_WHISPER, FF_Sender);
                        end
        ...
        if (event == "QUEST_ACCEPT_CONFIRM") then --When an escort quest is shared
            
            if (FF_Options_DualBox["AutoAcceptQuests"] == 1) then
                ConfirmAcceptQuest(); --Accept the escort quest
                FF_LocalMessage(FFMSG_QUEST_ACCEPTED);
            end
            
        end
        
        if (event == "QUEST_DETAIL") then --When any quest window (except escort quests) opens
        ...
        if (event == "UNIT_HEALTH") then
            ...
            if (arg1 == "player") then --If the health change is for the player (i.e. not for a group member etc)
                
                if (FF_Options_DualBox["AnnounceHealthWarning"] == 1) then
                    
                    if (UnitHealth("player") <= (UnitHealthMax("player") * 0.15)) then --If the players health is 15% or lower
                        
                        if (FFSentHealthWarning == false) then --If the health warning has not already been sent
                            
                            if (FF_Following == true) then
                                
                                SendAddonMessage(FF_AddonMessagePrefix, "LowHealth", "WHISPER", FF_Leader); --Send a warning message to the person being followed (This person must also have the addon installed)
                                
                                if ((string.len(FF_Master) > 0) and (string.lower(FF_Leader) ~= string.lower(FF_Master))) then --If a DualBox Master has been set and the master is not the person being followed
                                    SendAddonMessage(FF_AddonMessagePrefix, "LowHealth", "WHISPER", FF_Master); --Send a warning message to the DualBox Master (The master must also have the addon installed)
                                end
                                
                            else
                                
                                if (string.len(FF_Master) > 0) then --If a DualBox Master has been set
                                    SendAddonMessage(FF_AddonMessagePrefix, "LowHealth", "WHISPER", FF_Master); --Send a warning message to the DualBox Master (The master must also have the addon installed)
                                end
                                
                            end --End: if (FF_Following == true) then
                            
                            FFSentHealthWarning = true; --We have sent a low health warning
                        end --End: if (FFSentHealthWarning == false) then
                        
                    else --If the player has over 15% health
                        FFSentHealthWarning = false; --Reset the Warning flag
                    end
                    
                end --End: if (FF_Options_DualBox["AnnounceHealthWarning"] == 1) then
                
            end --End: if (arg1 == "player") then
            
        end --End: if (event == "UNIT_HEALTH") then
...
--            FF_Sender=arg2;
            ...
            --FFDebug("FF Disabled, but got event CHAT_MSG_WHISPER from "..FF_Sender);
...
end --End: function FF_OnEvent()

function FF_ParseWhisper(arg1, arg2)
--[[ NOTE: usage of FF_AuthoriseSender(Sender, Party(true/false), Raid(true/false), Guild(true/false), Friend(true/false), DualBox Master(true/fales));]]
        ...
        if (FF_Sender_Banned == false) then
            ...
            if ((string.sub(string.lower(arg1), 1, string.len(FFWHISP_FOLLOW)) == FFWHISP_FOLLOW) or (string.sub(string.lower(arg1), 1, string.len(FFWHISP_FOLLOW_ALIAS)) == FFWHISP_FOLLOW_ALIAS)) then --If the message is !ff_follow
                ...
                if (string.sub(string.lower(arg1), 1, string.len(FFWHISP_MOUNTFLYING)) ~= FFWHISP_MOUNTFLYING) then --FFWHISP_FOLLOW_ALIAS (!ff_f) can is called when !ff_fly is whispered. Make sure this wasn't !ff_fly
                    ...
                    FF_SenderID = FF_AuthoriseSender(FF_Sender, true, true, true, true, true);--Check if the player is a guild, party or raid member, a friend or the DualBox Master
                    ...
                    if (FF_SenderID ~= nil) then --If the sender was authorised
                        ...
                        if (FF_PermitFollow == 1) then
                            ...
                            if (string.len(FF_SenderID) > 4) then
                                
                                if ((string.sub(string.lower(FF_SenderID), 0, 5) == "party") or (string.sub(string.lower(FF_SenderID), 0, 4) == "raid")) then
                                    ...
                                    if (CheckInteractDistance(FF_SenderID, 4) == nil) then --Check if the player is close enough
                                        ...
                                        if (string.len(FF_LastFollowed) > 0) then --If necesarry, cancel auto resume follow to prevent spamming messages each time combat ends
                                            FF_CancelAutoResumeFollow(); --The player was not close enough to resume
                                        end
                                    ...
                                    end --End: if (CheckInteractDistance(FF_SenderID, 4) == nil) then
                        ...
                        FF_SendWhisper(FFMSG_CANT_FOLLOW..", "..FF_Sender, FF_Sender); --Tell the player you cant follow them
            ...
            if ((string.sub(string.lower(arg1), 1, string.len(FFWHISP_INVITE_ME)) == FFWHISP_INVITE_ME) or (string.sub(string.lower(arg1), 1, string.len(FFWHISP_INVITE_ME_ALIAS)) == FFWHISP_INVITE_ME_ALIAS)) then --If the message is !ff_inviteme
                ...
                FF_SenderID = FF_AuthoriseSender(FF_Sender, false, false, FF_Options["AllowInviteRequests"]["Guild"], FF_Options["AllowInviteRequests"]["Friends"], FF_Options["AllowInviteRequests"]["Master"]);--Check if the player is a guild member or a friend
                
                if (FF_SenderID ~= nil) then --If the sender was authorised
                        
                    FFAuthorised = FFWHISPLOG_YES;
                        
                    if (FF_Options["AllowInviteRequests"]["Enabled"] == 1) then
                        
                        if ((UnitIsGroupLeader("player")) or (GetNumSubgroupMembers() == 0)) then --If the player is group leader or  isnt in a party
                            
                            FF_SenderID = FF_AuthoriseSender(FF_Sender, true, false, false, false, false);--Check if the player is allready in our group
            ...
            if ((string.sub(string.lower(arg1), 1, string.len(FFWHISP_MOUNTGROUND)) == FFWHISP_MOUNTGROUND) or (string.sub(string.lower(arg1), 1, string.len(FFWHISP_MOUNTGROUND_ALIAS)) == FFWHISP_MOUNTGROUND_ALIAS)) then --If the message is !ff_mount or !ff_m
                FF_Sender=arg2; --Get name of sender
                FF_SenderID = FF_AuthoriseSender(FF_Sender, true, true, true, true, true);--Check if the player is a guild member, party/raid member or a friend
            ...
            if (string.sub(string.lower(arg1), 1, string.len(FFWHISP_SIT)) == FFWHISP_SIT) then --If the message is !ff_sitorstand
                ...
                FF_SenderID = FF_AuthoriseSender(FF_Sender, FF_Options["AllowEmoteCommand"]["Group"], FF_Options["AllowEmoteCommand"]["Raid"], FF_Options["AllowEmoteCommand"]["Guild"], FF_Options["AllowEmoteCommand"]["Friends"], FF_Options["AllowEmoteCommand"]["Master"]);--Check if the player is a guild member, party/raid member or a friend
            ...
            if (string.sub(string.lower(arg1), 1, string.len(FFWHISP_EMOTE)) == FFWHISP_EMOTE) then--If the message is !ff_emote
                ...
                FF_Sender=arg2;
                FF_SenderID = FF_AuthoriseSender(FF_Sender, FF_Options["AllowEmoteCommand"]["Group"], FF_Options["AllowEmoteCommand"]["Raid"], FF_Options["AllowEmoteCommand"]["Guild"], FF_Options["AllowEmoteCommand"]["Friends"], FF_Options["AllowEmoteCommand"]["Master"]);--Check if the player is a guild member or a friend
                ...
                if (FF_Options["AllowEmoteCommand"]["Enabled"] == 1) then
...
end --End: function FF_ParseWhisper()

function FF_ParseAddonMessage(event, arg1, arg2, arg3, arg4)
    ...
    --[[#######################################################
        ...
        if (string.sub(arg2, 1, 12) == "StatusReply:") then
            ...
            if (arg3 == "PARTY") then
                ...
                if (UnitName("player") == arg4) then --If the sender is ourself
                	return 0; --Exit the function to stop ourself being added to the table
                end
        ...
        if (arg2 == "StoppedFollowingYou") then
            ...
            NewFFWarning(arg4..FFMSG_STOPPED_FOLLOWING_YOU);
        end
        
        if (arg2 == "LowHealth") then
            FF_LocalMessage(arg4..FFMSG_HAS_LOW_HEALTH);
            NewFFWarning(arg4..FFMSG_HAS_LOW_HEALTH);
        end
        
    end --End: if (arg1 and arg2 and arg3 and arg4) then

end --End: function FF_ParseAddonMessage()

function FF_AuthoriseSender(FF_AuthSender, FF_AuthGroup, FF_AuthRaid, FF_AuthGuild, FF_AuthFriend, FF_AuthMaster)

    if (FF_AuthGroup == 1) then
        FF_AuthGroup = true;
    elseif (FF_AuthGroup == 0) then
        FF_AuthGroup = false;
    end
    
    if (FF_AuthRaid == 1) then
        FF_AuthRaid = true;
    elseif (FF_AuthRaid == 0) then
        FF_AuthRaid = false;
    end
    
    if (FF_AuthGuild == 1) then
        FF_AuthGuild = true;
    elseif (FF_AuthGuild == 0) then
        FF_AuthGuild = false;
    end
    
    if (FF_AuthFriend == 1) then
        FF_AuthFriend = true;
    elseif (FF_AuthFriend == 0) then
        FF_AuthFriend = false;
    end
    
    if (FF_AuthMaster == 1) then
        FF_AuthMaster = true;
    elseif (FF_AuthMaster == 0) then
        FF_AuthMaster = false;
    end

    if (FF_AuthGroup == true) then--If party should be checked
--        for i=1, GetNumSubgroupMembers() do --For every party member
--            
--            if (UnitName("party"..i) == FF_AuthSender) then --If the player is in the party
...
--        end
		...
		if (UnitPlayerOrPetInParty(FF_AuthSender) == true) then
			return FF_AuthSender;
		end
    end
    
    if (FF_AuthRaid == true) then--If raid should be checked
        for i=1, GetNumGroupMembers() do --For every raid member
        
            if (UnitName("raid"..i) == FF_AuthSender) then --If the player is in the raid
    ...
    if (FF_AuthGuild == true) then--If guild should be checked
        ...
        if (IsInGuild() == 1) then --If the player is in a guild
            ...
            for i=1, GetNumGuildMembers() do --for each guild member
                ...
                local FF_GuildMemberName = GetGuildRosterInfo(i); --Get guild member name
    ...
    if (FF_AuthFriend == true) then--If friend should be checked
        ...
        for i=1, GetNumFriends() do
            ...
            local FF_FriendName = GetFriendInfo(i); --Get the names of friends in the friend list
        ...
        --Check RealID Friends
        local FFBNTotalFriends, FFOnlineBNetFriends = BNGetNumFriends(); --Get Number of RealID friends
        for i=1, FFOnlineBNetFriends do
        
            local FF_PresenceID, FF_ForeName,FF_Surname, FF_ToonName = BNGetFriendInfo(i); --Get all of the toon names of the users RealID friends

            if (FF_ToonName == FF_AuthSender) then --if the sender is a RealID friend's toon
                return FF_AuthSender;
            end
            
        end
        
    end --End: if (FF_AuthFriend == true) then
    
    if (FF_AuthMaster == true) then --If the DualBox Master should be checked
        
        if (strlen(FF_Master) > 0) then
            
            if (string.lower(FF_AuthSender) == string.lower(FF_Master)) then --If the sender is the DualBox Master
                return FF_AuthSender;
            end
            
        end
        
    end --End: if (FF_AuthMaster == true) then
    
    return nil; --The player was not in the party, raid, guild or friends list
end --End: function FF_AuthoriseSender()