...
function addon:SaveTemplate(barID, templateName, overwrite)
    ...
    if self.db.global.templates[templateName] and not overwrite then
        ...
        for i = 1, self.maxButtons do
            if self.bars[barID].buttons[i].objective then
                self.db.global.templates[templateName][tostring(i)] = self.bars[barID].buttons[i].objective
            end
        end
...
StaticPopupDialogs["FARMINGBAR_CONFIRM_OVERWRITE_TEMPLATE"] = {
    text = L._StaticPopups("confirmOverwriteTemplate"),
    button1 = L["Yes"],
    button2 = L["No"],
...
StaticPopupDialogs["FARMINGBAR_INCLUDE_TEMPLATE_DATA"] = {
    text = L._StaticPopups("includeTemplateData"),
    button1 = L["Yes"],
    button2 = L["No"],
    button3 = L["Cancel"],
...
StaticPopupDialogs["FARMINGBAR_SAVE_TEMPLATE_ORDER"] = {
    text = L._StaticPopups("saveTemplateOrder"),
    button1 = L["Yes"],
    button2 = L["No"],
    button3 = L["Cancel"],
    OnAccept = function(_, data)
