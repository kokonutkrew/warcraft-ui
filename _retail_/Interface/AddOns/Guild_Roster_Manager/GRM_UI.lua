...
-- Core Frame
GRM_UI.GRM_MemberDetailMetaData = CreateFrame( "Frame" , "GRM_MemberDetailMetaData" , CommunitiesFrame , "TranslucentFrameTemplate" );
...
GRM_UI.GRM_MemberDetailMetaData.GRM_OfficerNoteTooltip = CreateFrame ( "GameTooltip" , "GRM_OfficerNoteTooltip" , CommunitiesFrameGuildDetailsFrameInfo , "GameTooltipTemplate" );
...
GRM_UI.GRM_CoreAltFrame.GRM_CoreAltScrollFrame.GRM_CoreAltScrollChildFrame = CreateFrame ( "Frame" , "GRM_CoreAltScrollChildFrame" );
-- SLIDER
...
GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadLogButton = CreateFrame( "Button" , "GRM_LoadLogButton" , CommunitiesFrame , "UIPanelButtonTemplate" );
...
GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadLogOldRosterButton = CreateFrame( "Button" , "GRM_LoadLogOldRosterButton" , GuildFrame , "UIPanelButtonTemplate" );
GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadLogOldRosterButtonText = GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadLogOldRosterButton:CreateFontString ( "GRM_LoadLogOldRosterButtonText" , "OVERLAY" , "GameFontWhiteTiny");
GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadLogOldRosterButton:Hide();

GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolButton = CreateFrame( "Button" , "GRM_LoadToolButton" , CommunitiesFrame , "UIPanelButtonTemplate" );
GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolButton:Hide();
GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolButtonText = GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolButton:CreateFontString ( "GRM_LoadToolButtonText" , "OVERLAY" , "GameFontWhiteTiny");

GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolOldRosterButton = CreateFrame( "Button" , "GRM_LoadToolOldRosterButton" , GuildFrame , "UIPanelButtonTemplate" );
GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolOldRosterButton:Hide();
GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolOldRosterButtonText = GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolOldRosterButton:CreateFontString ( "GRM_LoadToolOldRosterButtonText" , "OVERLAY" , "GameFontWhiteTiny");
...
GRM_UI.ReloadAllFrames = function( isManualUpdate )
    ...
    if CommunitiesFrame and CommunitiesFrame.GuildMemberDetailFrame and CommunitiesFrame.GuildMemberDetailFrame:IsVisible() then
    ...
    if isManualUpdate then
        ...
        if GRM_UI.GRM_ToolCoreFrame and GRM_UI.GRM_ToolCoreFrame.IsInitialized then
            ...
            -- Selection Scrollframe
            if GRM_UI.GRM_ToolCoreFrame.GRM_ToolSelectionScrollChildFrame.AllButtons ~= nil then
                for i = 1 , #GRM_UI.GRM_ToolCoreFrame.GRM_ToolSelectionScrollChildFrame.AllButtons do
                    GRM.BuildKickSelectionScrollButtons ( i , true )
                end
            end
...
GRM_UI.GR_MetaDataInitializeUIFirst = function( isManualUpdate )
    ...
    if not isManualUpdate then
        ...
        if not GuildMemberDetailFrame then
            GuildFrame:Show();
            GuildFrame:Hide();
        end
        ...
        CommunitiesFrame.GuildMemberDetailFrame:SetScript ( "OnShow" , function( self )
        ...
        CommunitiesFrame.GuildMemberDetailFrame:SetScript ( "OnHide" , function()
        ...
        CommunitiesFrame.GuildMemberDetailFrame:SetScript ( "OnKeyDown" , function ( self , key )
            self:SetPropagateKeyboardInput ( true );      -- Ensures keyboard access will default to the main chat window on / or Enter. UX feature.
            if key == "ESCAPE" then
                self:SetPropagateKeyboardInput ( false );
                self:Hide();
    ...
    if not isManualUpdate and CanEditMOTD() then
        ...
        CommunitiesFrameGuildDetailsFrameInfo.EditDetailsButton:HookScript ( "OnUpdate" , function( self , elapsed )
            officerTooltipTimer = officerTooltipTimer + elapsed;
            if officerTooltipTimer > 0.05 then
                if self:IsMouseOver() and not GRM_UI.GRM_MemberDetailMetaData.GRM_OfficerNoteTooltip:IsVisible() and not CommunitiesGuildTextEditFrame.Container.ScrollFrame.EditBox:HasFocus() then
                    -- Get officer note.
                    ...
                    if notes ~= nil and #notes > 0 then
                        ...
                        GRM_UI.GRM_MemberDetailMetaData.GRM_OfficerNoteTooltip:AddLine ( GRM.L ( "'g5^X+' -Use the +/- sign at the end to enable the use of the tag headers" ) );
                        GRM_UI.GRM_MemberDetailMetaData.GRM_OfficerNoteTooltip:AddLine ( GRM.L ( "'g6^Text^^Text^^' - Custom tags for note join dates - \"g6^Joined:^^Rejoined:^^\"" ) );
            ...
            if GRM_GuildMemberHistory_Save[ GRM_G.FID ][ GRM_G.saveGID ][j][1] == playerDetails.name then         -- Player Found and Located.
                ...
                for h = 1 , GRM.GetNumGuildies() do
                    ...
                    if playerName == playerDetails.name and officerNote ~= playerDetails.newNote and CanEditOfficerNote() then      -- No need to update old note if it is the same.
                        ...
                        if CommunitiesFrame.GuildMemberDetailFrame:IsVisible() then
                            if GRM.SlimName ( CommunitiesFrame.GuildMemberDetailFrame.Name:GetText() ) == GRM.SlimName ( GRM_G.currentName ) then
                                CommunitiesFrame.GuildMemberDetailFrame.OfficerNoteBackground.OfficerNoteText:SetText( theNote );
                            end
                        end
    ...
    GRM_UI.GRM_MemberDetailMetaData.GRM_SafeFromRulesCheckButton:SetScript ( "OnEnter" , function( self )
        GRM_UI.SetTooltipScale();
        GameTooltip:SetOwner ( self , "ANCHOR_CURSOR" );
...
GRM_UI.GR_MetaDataInitializeUISecond = function( isManualUpdate )
    ...
    GRM_UI.GRM_CoreAltFrame.GRM_CoreAltScrollFrame:SetScrollChild ( GRM_UI.GRM_CoreAltFrame.GRM_CoreAltScrollFrame.GRM_CoreAltScrollChildFrame );
    -- Slider Parameters
...
GRM_UI.GR_MetaDataInitializeUIThird = function( isManualUpdate )
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadLogButton:SetPoint ( "BOTTOMRIGHT" , CommunitiesFrame , "TOPRIGHT" , 0 , 1 );
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadLogOldRosterButton:SetScript ( "OnClick" , function ( _ , button)
        if button == "LeftButton" then
            if GRM_UI.GRM_RosterChangeLogFrame:IsVisible() then
                GRM_UI.GRM_RosterChangeLogFrame:Hide();
            else
    ...
    -- BUTTONS
    GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolButton:SetSize ( 90 , 11 );
    GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolButton:SetPoint ( "RIGHT" , GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadLogButton , "LEFT" , 0 , 0 );
    GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolButton.Timer = 0;
    GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolButtonText:SetPoint ( "CENTER" , GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolButton );
    GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolButtonText:SetFont ( GRM_G.FontChoice , GRM_G.FontModifier + 8 );
    GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolButtonText:SetText ( GRM.L ( "Macro Tool" ) );

    GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolButton:SetScript ( "OnClick" , function ( _ , button)
        if button == "LeftButton" then
            GRM.SlashCommandKick();
        end
    end);

    GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolButton:SetScript ( "OnShow" , function()
        GRM_UI.RefreshToolButtonsOnUpdate();
    end);

    GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolButton:SetScript ( "OnUpdate" , function ( self , elapsed )
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolOldRosterButton:SetScript ( "OnClick" , function ( _ , button)
        if button == "LeftButton" then
            GRM.SlashCommandKick();
        end
    end);

    GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolOldRosterButton:SetScript ( "OnShow" , function()
        GRM_UI.RefreshToolButtonsOnUpdate();
    end);

    GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolOldRosterButton:SetScript ( "OnUpdate" , function ( self , elapsed )
        if not GRM_UI.GRM_RosterChangeLogFrame.GRM_LoadToolButton:IsVisible() then
            self.Timer = self.Timer + elapsed;
            if self.Timer > 60 then       -- Only update once per minute
                GRM_UI.RefreshToolButtonsOnUpdate();
                self.Timer = 0;
    ...
    if not isManualUpdate then
        ...
        CommunitiesFrameGuildDetailsFrameInfo.EditMOTDButton:HookScript ( "OnClick" , function( _ , button )
            if button == "LeftButton" then
                CommunitiesGuildTextEditFrame.Container.ScrollFrame.EditBox:HighlightText( 0 );
                CommunitiesGuildTextEditFrame.GuildMOTDcharCount:SetText( #CommunitiesGuildTextEditFrame.Container.ScrollFrame.EditBox:GetText() .. "/" .. CommunitiesGuildTextEditFrame.Container.ScrollFrame.EditBox:GetMaxLetters() );
                CommunitiesGuildTextEditFrame.GuildMOTDcharCount:Show();
            end
        end);

        CommunitiesGuildTextEditFrame.Container.ScrollFrame.EditBox:HookScript ( "OnEditFocusLost" , function()
            CommunitiesGuildTextEditFrame.GuildMOTDcharCount:Hide();
        end);

        CommunitiesGuildTextEditFrame.Container.ScrollFrame.EditBox:HookScript ( "OnEditFocusGained" , function()
            CommunitiesGuildTextEditFrame.GuildMOTDcharCount:Show();
        end);

        CommunitiesGuildTextEditFrame.Container.ScrollFrame.EditBox:HookScript ( "OnTextChanged" , function( self )
            CommunitiesGuildTextEditFrame.GuildMOTDcharCount:SetText( #self:GetText() .. "/" .. self:GetMaxLetters() );
        end)
        
        CommunitiesFrameGuildDetailsFrameInfo.EditDetailsButton:HookScript ( "OnClick" , function( _ , button )
            if button == "LeftButton" then
                CommunitiesGuildTextEditFrame.GuildMOTDcharCount:SetText( #CommunitiesGuildTextEditFrame.Container.ScrollFrame.EditBox:GetText() .. "/" .. CommunitiesGuildTextEditFrame.Container.ScrollFrame.EditBox:GetMaxLetters() );
                CommunitiesGuildTextEditFrame.GuildMOTDcharCount:Show();
            end
    ...
    CommunitiesGuildTextEditFrame.GuildMOTDcharCount = CommunitiesGuildTextEditFrame:CreateFontString ( "GuildMOTDcharCount" , "OVERLAY" , "GameFontNormalSmall" );
    CommunitiesGuildTextEditFrame.GuildMOTDcharCount:SetPoint ( "TOPRIGHT" , CommunitiesGuildTextEditFrame , -33 , -18 )
    CommunitiesGuildTextEditFrame.GuildMOTDcharCount:SetFont ( GRM_G.FontChoice , GRM_G.FontModifier + 12 );

    CommunitiesGuildTextEditFrame:HookScript ( "OnHide" , function()
        GRM.UpdateGuildLeaderPermissions( true );
    end);
end

-- Method:          GRM.PreAddonLoadUI( bool )
...
GRM_UI.PreAddonLoadUI = function( isManualUpdate )
    ...
    -- Add Event Frame
    GRM_UI.GRM_RosterChangeLogFrame.GRM_BanListTab:SetScript ( "OnUpdate" , function( _ , elapsed )
...
GRM_UI.MetaDataInitializeUIrosterLog1 = function( isManualUpdate )
    ...
    if not isManualUpdate then
        GRM_UI.GRM_MemberDetailMetaData.GRM_MemberDetailRankToolTip:SetScale ( 0.65 );
        GRM_UI.GRM_MemberDetailMetaData.GRM_MemberDetailJoinDateToolTip:SetScale ( 0.65 );
    ...
    GRM_UI.LogEditBoxTimingControl = function()
        if time() - GRM_G.logTimer >= 0.2 and GRM_UI.GRM_RosterChangeLogFrame.GRM_LogFrame.GRM_LogEditBox.MaxDelayCount ~= GRM_UI.GRM_RosterChangeLogFrame.GRM_LogFrame.GRM_LogEditBox.currentCount then
            GRM_G.logTimer = time();
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_LogFrame.GRM_LogExtraOptionsFrame.GRM_LogShowTooltipCheckButtonText:SetPoint ( "LEFT" , GRM_UI.GRM_RosterChangeLogFrame.GRM_LogFrame.GRM_LogExtraOptionsFrame.GRM_LogShowTooltipCheckButton , 27 , 0 );
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_LogFrame.GRM_ExportLogBorderFrame:ClearAllPoints();
    GRM_UI.GRM_RosterChangeLogFrame.GRM_LogFrame.GRM_ExportLogBorderFrame:SetPoint ( "CENTER" , UIParent );
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_UIOptionsFrame.GRM_MemoryUsageText:SetPoint ( "TOPRIGHT" , GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.OptionsHeaderText3 , "BOTTOMRIGHT" , -10 , -31 )
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_UIOptionsFrame.GRM_AutoBackupTimeEditBox:SetPoint ( "LEFT" , GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_UIOptionsFrame.DaysOnAutoBackupText , "RIGHT" , 5 , 0 );
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_UIOptionsFrame.GRM_AutoBackupTimeEditBox:SetSize ( 25 , 22 );
    ...
    -- Report Destination
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_GeneralOptionsFrame.GRM_ReportDestinationText:SetPoint ( "TOPLEFT" , GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_GeneralOptionsFrame.GRM_SyncAllSettingsCheckButton , "BOTTOMLEFT" , 5 , -12 );
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_GeneralOptionsFrame.GRM_ReportDestinationEditBox:SetSize ( 115 , 22 );
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_GeneralOptionsFrame.GRM_ReportDestinationEditBox:SetMaxLetters ( 31 );
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_GeneralOptionsFrame.GRM_ReportDestinationEditBox:SetFont ( GRM_G.FontChoice , GRM_G.FontModifier + 12 );
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_GeneralOptionsFrame.GRM_ReportDestinationEditBox:SetScript ( "OnEnterPressed" , function ( self )
        ...
        self:ClearFocus();
        self:SetText( text );  -- Need to re-add
    end);
    ...
    -- LANGUAGE SELECTION
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_GeneralOptionsFrame.GRM_LanguageSelectionText:SetPoint ( "TOPLEFT" , GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_GeneralOptionsFrame.GRM_ReportDestinationText , "BOTTOMLEFT" , 0 , -16 );
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_GeneralOptionsFrame.GRM_TimestampSelected:SetScript ( "OnEnter" , function ( self )
        local isRestricted = GRM.IsSyncRankGuildLeaderRestricted ( 0 , "g1^" );
    ...
    GRM_UI.RadialButtonTooltipGlobalControlsConfig = function ( frame , tag , includeCheckbox )
    ...
    GRM_UI.RadialButtonTooltipGlobalControlsConfig ( GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_OfficerOptionsFrame.GRM_RosterAddTimestampCheckButton , "g5^" , true );
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_OfficerOptionsFrame.GRM_AddJoinedTagButton:SetScript ( "OnEnter" , function ( self )
        local isRestricted = GRM.IsSyncRankGuildLeaderRestricted ( 0 , "g5^" , true );
        if isRestricted then
    ...
    -- Report Destination
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_OfficerOptionsFrame.GRM_CustomTagJoinText:SetPoint ( "TOPLEFT" , GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_OfficerOptionsFrame.GRM_AddJoinedTagButtonText , "BOTTOMLEFT" , 0 , -16 );
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_OfficerOptionsFrame.GRM_CustomTagJoinEditBox:SetPoint ( "LEFT" , GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_OfficerOptionsFrame.GRM_CustomTagJoinText , "RIGHT" , 8 , 0 );
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_OfficerOptionsFrame.GRM_CustomTagJoinEditBox:SetScript ( "OnEnter" , function ( self )
        if not self:HasFocus() then
            ...
            if GRM.IsCustomJoinRestricted() then
                GameTooltip:AddLine ( GameTooltip:AddLine ( "|CFF00CCFF" .. GRM.L ( "Warning - Global Controls:" ) ) );
                GameTooltip:AddLine ( GRM.L ( "Unable to Modify. Global setting is set to :   {name}" , GRM_G.customHeaderJoin .. " " .. GRM.FormatTimeStamp ( GRM.GetTimestamp() , false ) ) );
                self:Disable();
            else
                GameTooltip:AddLine ( GRM_G.customHeaderJoin .. " " .. GRM.FormatTimeStamp ( GRM.GetTimestamp() , false ) );
                self:Enable();
            end
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_OfficerOptionsFrame.GRM_CustomTagREJoinText:SetPoint ( "LEFT" , GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_OfficerOptionsFrame.GRM_CustomTagJoinEditBox , "RIGHT" , 8 , 0 );
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_OfficerOptionsFrame.GRM_CustomTagREJoinEditBox:SetScript ( "OnEnter" , function( self )
        if not self:HasFocus() then
            ...
            if GRM.IsCustomReJoinRestricted() then
                GameTooltip:AddLine ( GameTooltip:AddLine ( "|CFF00CCFF" .. GRM.L ( "Warning - Global Controls:" ) ) );
                GameTooltip:AddLine ( GRM.L ( "Unable to Modify. Global setting is set to :   {name}" , GRM_G.customHeaderRejoin .. " " .. GRM.FormatTimeStamp ( GRM.GetTimestamp() , false ) ) );
                self:Disable();
            else
                GameTooltip:AddLine ( GRM_G.customHeaderRejoin .. " " .. GRM.FormatTimeStamp ( GRM.GetTimestamp() , false ) );
                self:Enable();
            end
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_OfficerOptionsFrame.GRM_UnifySettingsCoreText:SetText ( 
        ...
        GRM.L ( "'g5^X+' -Use the +/- sign at the end to enable the use of the tag headers" ) .. "\n" ..
        GRM.L ( "'g6^Text^^Text^^' - Custom tags for note join dates - \"g6^Joined:^^Rejoined:^^\"" )
    );
...
GRM_UI.MetaDataInitializeUIrosterLog2 = function( isManualUpdate )
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_OfficerOptionsFrame.GRM_RecruitNotificationCheckButton:SetPoint ( "TOPRIGHT" , GRM_UI.GRM_RosterChangeLogFrame.GRM_OptionsFrame.GRM_OfficerOptionsFrame.GRM_CustomTagJoinText , "BOTTOMLEFT" , 0 , -12 );
    ...
    GRM_UI.GRM_RosterChangeLogFrame:SetScript ( "OnKeyDown" , function ( _ , key )
        ...
        if key == "ESCAPE" then
            ...
            if GRM_UI.GRM_MemberDetailMetaData:IsVisible() then
                ...
                -- Blizz default frame
                elseif CommunitiesFrame.GuildMemberDetailFrame ~= nil and CommunitiesFrame.GuildMemberDetailFrame:IsVisible() then
                    CommunitiesFrame.GuildMemberDetailFrame:Hide();
                    GRM_G.pause = true;
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_AuditFrame.GRM_AuditFrameButton1:SetScript ( "OnClick" , function ( _ , button )
        if button == "LeftButton" then
            if ( time() - GRM_G.sortRestrictionTimer ) >= 2 then                                     -- For triggering the sorting
                ...
                GRM.Report ( GRM.L ( "Please Wait {num} more Seconds" , nil , nil , math.floor ( 2 - ( time()-GRM_G.sortRestrictionTimer ) ) ) );
            end
        end
    end);
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_AuditFrame.GRM_JDAuditToolButton:SetPoint ( "TOP" , GRM_UI.GRM_RosterChangeLogFrame.GRM_AuditFrame , "BOTTOM" , -87 , 2 );
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_AuditFrame.GRM_AuditScrollFrameSlider:SetValueStep ( 22.1875 );
    GRM_UI.GRM_RosterChangeLogFrame.GRM_AuditFrame.GRM_AuditScrollFrameSlider:SetStepsPerPage ( 16 );
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_AuditFrame.GRM_AuditScrollFrameSlider:SetScript( "OnValueChanged" , function( self , value )
        ...
        GRM.HybridScrollOnValueChangedConfig (
            ...
            16 , 22.1875 , GRM.RefreshAuditFrames , GRM_G.AuditEntries
        );
    ...
    -- Fontstrings
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText1:SetPoint ( "BOTTOMLEFT" , GRM_AuditJDTool.GRM_JDToolScrollFrame , "TOPLEFT" , 10  , 6 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText1:SetFont ( GRM_G.FontChoice , GRM_G.FontModifier + 14 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText1:SetWidth ( 115 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText1:SetJustifyH ( "LEFT" );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText1:SetText ( GRM.L ( "Name" ) );

    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText2:SetPoint ( "LEFT" , GRM_AuditJDTool.GRM_AuditToolText1 , "RIGHT" , 10 , 0 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText2:SetFont ( GRM_G.FontChoice , GRM_G.FontModifier + 14 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText2:SetWidth ( 100 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText2:SetJustifyH ( "CENTER" );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText2:SetText ( GRM.L ( "GRM Date" ) );

    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText3:SetPoint ( "LEFT" , GRM_AuditJDTool.GRM_AuditToolText2 , "RIGHT" , 10 , 0 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText3:SetFont ( GRM_G.FontChoice , GRM_G.FontModifier + 14 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText3:SetWidth ( 100 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText3:SetJustifyH ( "CENTER" );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText3:SetText ( GRM.L ( "Note Date" ) );

    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText4:SetPoint ( "LEFT" , GRM_AuditJDTool.GRM_AuditToolText3 , "RIGHT" , 0 , 0 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText4:SetFont ( GRM_G.FontChoice , GRM_G.FontModifier + 14 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText4:SetWidth ( 100 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText4:SetJustifyH ( "CENTER" );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText4:SetText ( GRM.L ( "Location" ) );

    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText5:SetPoint ( "LEFT" , GRM_AuditJDTool.GRM_AuditToolText4 , "RIGHT" , 10 , 0 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText5:SetFont ( GRM_G.FontChoice , GRM_G.FontModifier + 14 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText5:SetWidth ( 220 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText5:SetJustifyH ( "LEFT" );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText5:SetText ( GRM.L ( "Status" ) );

    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText6:SetPoint ( "LEFT" , GRM_AuditJDTool.GRM_AuditJDToolButton6 , "RIGHT" , 5, 0 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText6:SetFont ( GRM_G.FontChoice , GRM_G.FontModifier + 14 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText6:SetWidth ( 410 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText6:SetJustifyH ( "CENTER" );

    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText7:SetPoint ( "LEFT" , GRM_UI.GRM_AuditJDTool.GRM_AuditJDToolCheckBoxText , "RIGHT" , 25  , 0 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText7:SetFont ( GRM_G.FontChoice , GRM_G.FontModifier + 11 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText7:SetJustifyH ( "LEFT" );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText7:SetTextColor ( 1 , 0 , 0 );
    GRM_UI.GRM_AuditJDTool.GRM_AuditToolText7:SetText ( GRM.L ( "Warning! Your function is extremely limited without officer permissions" ) );

    GRM_UI.GRM_AuditJDTool.GRM_AuditJDToolButton7Text:SetPoint ( "CENTER" , GRM_AuditJDTool.GRM_AuditJDToolButton7 );
    ...
    GRM_UI.GRM_AuditJDTool.GRM_AuditJDToolButton1Text:SetPoint ( "CENTER" , GRM_AuditJDTool.GRM_AuditJDToolButton1 );
    ...
    GRM_UI.GRM_AuditJDTool.GRM_AuditJDToolButton2Text:SetPoint ( "CENTER" , GRM_AuditJDTool.GRM_AuditJDToolButton2 );
    ...
    GRM_UI.GRM_AuditJDTool.GRM_AuditJDToolButton3Text:SetPoint ( "CENTER" , GRM_AuditJDTool.GRM_AuditJDToolButton3 );
    ...
    GRM_UI.GRM_AuditJDTool.GRM_AuditJDToolButton4Text:SetPoint ( "CENTER" , GRM_AuditJDTool.GRM_AuditJDToolButton4 );
    ...
    GRM_UI.GRM_AuditJDTool.GRM_AuditJDToolButton5Text:SetPoint ( "CENTER" , GRM_AuditJDTool.GRM_AuditJDToolButton5 );
    ...
    GRM_UI.GRM_AuditJDTool.GRM_AuditJDToolButton6Text:SetPoint ( "CENTER" , GRM_AuditJDTool.GRM_AuditJDToolButton6 );
    ...
    GRM_UI.GRM_AuditJDTool.GRM_AuditJDToolCheckBoxText:SetPoint ( "LEFT" , GRM_UI.GRM_AuditJDTool.GRM_AuditJDToolCheckBox , 27 , 0 );
    ...
    GRM_UI.GRM_AuditJDTool.GRM_AuditJDToolButton2:SetScript ( "OnClick" , function ( _ , button ) 
        if button == "LeftButton" then
            if GRM.GetNumMismatchedButton2() > 0 then
                GRM.SetConfirmationWindow ( GRM.MismatchConfirmConfig3 , GRM.L ( "Do you really want to import all dates from the notes?" ) );
            else
    ...
    GRM_UI.GRM_AuditJDTool.GRM_AuditJDToolButton2:SetScript ( "OnEnter" , function ( self )
        GRM_UI.SetTooltipScale();
        GameTooltip:SetOwner ( self , "ANCHOR_CURSOR" );
        GameTooltip:AddLine( GRM.L ( "There are currently {num} dates that can be imported" , nil , nil , GRM.GetNumMismatchedButton2() ) );
        GameTooltip:Show();
    end);
    ...
    GRM_UI.GRM_AuditJDTool.GRM_AuditJDToolButton1:SetScript ( "OnEnter" , function ( self )
        GRM_UI.SetTooltipScale();
        GameTooltip:SetOwner ( self , "ANCHOR_CURSOR" );
        GameTooltip:AddLine( GRM.L ( "There are currently {num} dates that can be added to the default note" , nil , nil , GRM.GetNumMismatchedButton1() ) );
        GameTooltip:Show();
    end);
    ...
    GRM_UI.GRM_AuditJDTool.GRM_AuditJDToolButton7:SetScript ( "OnClick" , function ( _ , button ) 
        if button == "LeftButton" then
            if GRM.GetNumMismatchedButton7() > 0 then
    ...
    GRM_UI.GRM_RosterChangeLogFrame.GRM_EventsFrame.GRM_AddEventScrollChildFrame:SetScript ( "OnDragStart" , function( self )
        GRM_UI.GRM_RosterChangeLogFrame:StartMoving();
    end);
    ...
    GRM_UI.GRM_GeneralPopupWindow:SetSize ( 265 , 105 );
    ...
    GRM_UI.GRM_GeneralPopupWindow.GRM_GeneralPopupWindowText:SetWidth ( 215 );
...
if GRM_G.ExpansionLvl > 1 then  -- 0 = Classic and 1 = TBC - no calendar yet existed
...
GRM_UI.RequestToJoinLogic_OnShow = function()
    ...
    if not CommunitiesGuildRecruitmentFrame.GRM_InviteMessageFrame.GRM_InviteMsgEditBox then
        ...
        CommunitiesGuildRecruitmentFrame.GRM_InviteMessageFrame.GRM_InviteMsgEditBox:SetScript ( "OnEditFocusGained" , function ( self )
            ...
            CommunitiesGuildRecruitmentFrame.GRM_InviteMessageFrame.GRM_InviteMsgEditBox.GRM_MsgEditBoxCharCount:SetText ( #self:GetText() .. "/" .. self:GetMaxLetters() );
...
GRM_UI.OldRosterLog_OnShow = function()
    if not GuildRosterFrame.GRM_EnableMouseOverOldRoster then
        GuildRosterFrame.GRM_EnableMouseOverOldRoster = CreateFrame ( "CheckButton" , "GRM_EnableMouseOverOldRoster" , GuildRosterFrame , "OptionsSmallCheckButtonTemplate" );
        GuildRosterFrame.GRM_EnableMouseOverOldRosterText = GuildRosterFrame.GRM_EnableMouseOverOldRoster:CreateFontString ( "GRM_EnableMouseOverOldRosterText" , "OVERLAY" , "GameFontNormalSmall" );

        GuildRosterFrame.GRM_EnableMouseOverOldRoster:SetSize ( 20 , 20 );
        GuildRosterFrame.GRM_EnableMouseOverOldRoster:ClearAllPoints();
        GuildRosterFrame.GRM_EnableMouseOverOldRoster:SetPoint ( "BOTTOMRIGHT" , GuildRosterViewDropdown , "TOPRIGHT" , -18 , -5.5 );
        GuildRosterFrame.GRM_EnableMouseOverOldRoster:SetHitRectInsets ( -90 , 1 , 1 , 1 );
        GuildRosterFrame.GRM_EnableMouseOverOldRosterText:SetPoint ( "RIGHT" , GuildRosterFrame.GRM_EnableMouseOverOldRoster , "LEFT" , -3 , 0 );
        GuildRosterFrame.GRM_EnableMouseOverOldRosterText:SetFont ( GRM_G.FontChoice , GRM_G.FontModifier + 9.5 );
        GuildRosterFrame.GRM_EnableMouseOverOldRosterText:SetText ( GRM.L ( "Show Mouseover" ) );
        GuildRosterFrame.GRM_EnableMouseOverOldRoster:SetScript ( "OnClick", function( self )
            if self:GetChecked() then
        ...
        if GRM_AddonSettings_Save[GRM_G.FID][GRM_G.setPID][2][71] then
            GuildRosterFrame.GRM_EnableMouseOverOldRoster:SetChecked ( true );
        end
    end
end
