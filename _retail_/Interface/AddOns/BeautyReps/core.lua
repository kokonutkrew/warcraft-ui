...
(____/(____)\_/\_/\____/ (__) (__/    (__\_)(____)(__)  (____/
												...
												Author: https://wow.curseforge.com/members/birdfriender
												Original Author: https://wow.curseforge.com/members/Sida2
												Curse: https://wow.curseforge.com/projects/Beauty-reps

]]


BeautyRepsDB = {Structure = {}, Options = {}}
local gender
...
local frame = CreateFrame("FRAME", "BeautyRepsEventFrame")
...
frame:SetScript("OnEvent", function(self, event, ...)
	if event == "PLAYER_LEAVING_WORLD" or event == "PLAYER_LOGOUT" then
		...
		if not loadDone then	
			playerOptions = BeautyRepsDB.Options or playerOptions
			fakeHeadersCollapsedStatus = BeautyRepsDB.Headers or fakeHeadersCollapsedStatus
			...
			--CharacterFrameTab2:SetText("Beauty Reps")
...
function Save()
	if characterRepStructure then
		BeautyRepsDB.Structure = characterRepStructure
		BeautyRepsDB.Options = playerOptions
		BeautyRepsDB.Headers = fakeHeadersCollapsedStatus
	end
end
...
--[[
 ...
 ____  ____  ____  _  _  ____  __  ____  __  __   __ _    _  _  __  
(  _ \(  __)(  _ \/ )( \(_  _)/ _\(_  _)(  )/  \ (  ( \  / )( \(  ) 
 )   / ) _)  ) __/) \/ (  )( /    \ )(   )((  O )/    /  ) \/ ( )(  
(__\_)(____)(__)  \____/ (__)\_/\_/(__) (__)\__/ \_)__)  \____/(__) 
		...
		if ( factionIndex <= numFactions ) then
			...
			if selectedFaction and factionData == selectedFaction then
				...
				if factionData.isGuild then
					BeautyRepsFavouriteFrame:Hide()
				else
					BeautyRepsFavouriteFrame:Show()
					BeautyRepsFavouriteStar:SetChecked(selectedFaction.isFavourite)
				end

				if not hasEncountered then
					ReputationDetailInactiveCheckBox:Hide()
					ReputationDetailMainScreenCheckBox:Hide()
					BeautyRepsEncounterFrame:Show()

					if isInactive then
						BeautyRepInactiveButton:SetChecked(1)
					else
						BeautyRepInactiveButton:SetChecked(nil)
					end
				else
					ReputationDetailInactiveCheckBox:Show()
					ReputationDetailMainScreenCheckBox:Show()
					BeautyRepsEncounterFrame:Hide()
...
--[[
 ...
 ____   ___  ____  __  ____  ____    _  _   __    __  __ _  ____ 
/ ___) / __)(  _ \(  )(  _ \(_  _)  / )( \ /  \  /  \(  / )/ ___)
\___ \( (__  )   / )(  ) __/  )(    ) __ ((  O )(  O ))  ( \___ \
(____/ \___)(__\_)(__)(__)   (__)   \_)(_/ \__/  \__/(__\_)(____/                                                                      
...
--[[
 ...
 ____   __  ____  __     _  _   __   __ _   __    ___  ____  _  _  ____  __ _  ____ 
(    \ / _\(_  _)/ _\   ( \/ ) / _\ (  ( \ / _\  / __)(  __)( \/ )(  __)(  ( \(_  _)
 ) D (/    \ )( /    \  / \/ \/    \/    //    \( (_ \ ) _) / \/ \ ) _) /    /  )(  
(____/\_/\_/(__)\_/\_/  \_)(_/\_/\_/\_)__)\_/\_/ \___/(____)\_)(_/(____)\_)__) (__)                                                 
...
function MergeRepData(characterStructuredData)
	local function Merge(characterFaction)
		local dbFaction = ExtractRepData(BeautyRepsDB.Structure, characterFaction.factionID)
		if dbFaction then
...
function ReloadCharacterData(saveFirst)
	...
	characterRepStructure = CopyTable(_G.BeautyReps.Structure)
	local guildName = GetGuildInfo("player")
...
function ModifyRepPanel()
	...
	do
		if not BeautyRepsEncounterFrame then
			CreateFrame("FRAME", "BeautyRepsEncounterFrame", ReputationDetailFrame)
			BeautyRepsEncounterFrame:CreateFontString("BeautyRepsEncounterText", "OVERLAY", "GameFontNormalSmall")

			BeautyRepsEncounterFrame:SetPoint("BOTTOMLEFT", ReputationDetailFrame, "BOTTOMLEFT", 12, 10)
			BeautyRepsEncounterFrame:SetSize(188, 50)
			BeautyRepsEncounterFrame:Hide()

			BeautyRepsEncounterText:SetPoint("CENTER", BeautyRepsEncounterFrame, "CENTER", 0, -10)
			BeautyRepsEncounterText:SetText("This character has not yet\nencountered this faction.")

			local checkbox = CreateFrame("CheckButton", "BeautyRepInactiveButton", BeautyRepsEncounterFrame, "OptionsSmallCheckButtonTemplate")
			checkbox:SetSize(26, 26)
			checkbox:SetPoint("TOPLEFT", ReputationDetailFrame, "TOPLEFT", 14, -143)
			BeautyRepInactiveButtonText:SetText("Move to Inactive")
			BeautyRepInactiveButton:SetScript("OnClick", InactiveCheckbox_Click)
	...
	-- Favourites Star
	do
		if not BeautyRepsFavouriteFrame then
			CreateFrame("FRAME", "BeautyRepsFavouriteFrame", ReputationDetailFrame)
			BeautyRepsFavouriteFrame:SetSize(7, 7)
			BeautyRepsFavouriteFrame:SetPoint("BOTTOMRIGHT", ReputationDetailFrame, "BOTTOMRIGHT", -25, 45)

			local checkbox = CreateFrame("CheckButton", "BeautyRepsFavouriteStar", BeautyRepsFavouriteFrame, "ChatConfigCheckButtonTemplate")
			...
			checkbox:SetPoint("CENTER", BeautyRepsFavouriteFrame)
	...
	-- Tab Frame
	do
		if not BeautyRepsTabFrame then
			CreateFrame("FRAME", "BeautyRepsTabFrame", ReputationFrame)
			BeautyRepsTabFrame:CreateFontString("BeautyRepsExaltedText", "ARTWORK", "GameFontHighlight")
			BeautyRepsTabFrame:SetPoint("TOPLEFT", ReputationFrame, "TOPLEFT", 0, -20)
			BeautyRepsTabFrame:SetSize(335, 50)
			BeautyRepsExaltedText:SetPoint("CENTER", BeautyRepsTabFrame, "CENTER", 0, 0)
			ReputationFrameStandingLabel:Hide()
			ReputationFrameFactionLabel:Hide()
		end	
	end

	if not BeautyRepsOptionsButton then
		CreateFrame("Button", "BeautyRepsOptionsButton", ReputationFrame)
		BeautyRepsOptionsButton:SetPoint("TOPRIGHT", ReputationFrame, "TOPRIGHT", -4, -30)
		BeautyRepsOptionsButton:SetSize(30, 30)
		BeautyRepsOptionsButton:SetNormalTexture("Interface/Buttons/UI-SpellbookIcon-NextPage-Up")
		BeautyRepsOptionsButton:SetPushedTexture("Interface/Buttons/UI-SpellbookIcon-NextPage-Down")
		BeautyRepsOptionsButton:SetDisabledTexture("Interface/Buttons/UI-SpellbookIcon-NextPage-Disabled")
		BeautyRepsOptionsButton:SetHighlightTexture("Interface/Buttons/UI-Common-MouseHilight", "ADD")
		BeautyRepsOptionsButton:SetScript("OnClick", function()
			ReputationDetailFrame:Hide()
			BeautyRepsOptionsFrame:Show()
			BeautyRepsOptionsButton:Hide()
			BeautyRepsOptionsHideButton:Show()
			PlaySound(SOUNDKIT.IG_CHARACTER_INFO_OPEN)
		end)

		CreateFrame("Button", "BeautyRepsOptionsHideButton", ReputationFrame)
		BeautyRepsOptionsHideButton:SetPoint("TOPRIGHT", ReputationFrame, "TOPRIGHT", -4, -30)
		BeautyRepsOptionsHideButton:SetSize(30, 30)
		BeautyRepsOptionsHideButton:SetNormalTexture("Interface/Buttons/UI-SpellbookIcon-PrevPage-Up")
		BeautyRepsOptionsHideButton:SetPushedTexture("Interface/Buttons/UI-SpellbookIcon-PrevPage-Down")
		BeautyRepsOptionsHideButton:SetDisabledTexture("Interface/Buttons/UI-SpellbookIcon-PrevPage-Disabled")
		BeautyRepsOptionsHideButton:SetHighlightTexture("Interface/Buttons/UI-Common-MouseHilight", "ADD")
		BeautyRepsOptionsHideButton:Hide()
		BeautyRepsOptionsHideButton:SetScript("OnClick", function()
			BeautyRepsOptionsFrame:Hide()
			BeautyRepsOptionsButton:Show()
			BeautyRepsOptionsHideButton:Hide()
			PlaySound(SOUNDKIT.IG_CHARACTER_INFO_CLOSE)
		end)
	end

	if not BeautyRepsOptionsFrame then
		local frameWidth, frameHeight = 250, 390
		CreateFrame("FRAME", "BeautyRepsOptionsFrame", ReputationFrame, "UIPanelDialogTemplate")
		BeautyRepsOptionsFrame:SetPoint("TOPLEFT", ReputationFrame, "TOPRIGHT", -15, -30)
		BeautyRepsOptionsFrame:SetSize(frameWidth, frameHeight)
		BeautyRepsOptionsFrame:SetFrameStrata("LOW")
		BeautyRepsOptionsFrame:Hide()

		-- Close Button
		BeautyRepsOptionsFrameClose:SetScript("OnClick", function()
			BeautyRepsOptionsFrame:Hide()
			BeautyRepsOptionsButton:Show()
			BeautyRepsOptionsHideButton:Hide()
			PlaySound(SOUNDKIT.IG_CHARACTER_INFO_CLOSE)
		...
		-- Title
		local title = BeautyRepsOptionsFrame:CreateFontString(nil, "OVERLAY", "GameFontHighlightCenter")
		title:SetPoint("CENTER", BeautyRepsOptionsFrame, "TOP", 0, -16)
		title:SetText("Beauty Reps")
		...
		local optionsTitle = BeautyRepsOptionsFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalCenter")
		optionsTitle:SetPoint("CENTER", BeautyRepsOptionsFrame, "TOP", -15, -50)
		optionsTitle:SetText("Configure Beauty Reps Tab")

		-- Options Help
		local helpButton = CreateFrame("Button", nil, BeautyRepsOptionsFrame, "UIPanelInfoButton")
		helpButton:SetPoint("TOPRIGHT", BeautyRepsOptionsFrame, -12, -35)
		local helpTooltip = "Beauty Reps can only include factions that have been encountered by at least one character.\n\n"
		...
		yPos = yPos - itemSpacing
		-- Header Totals
		checkbox = CreateFrame("CheckButton", nil, BeautyRepsOptionsFrame, "ChatConfigCheckButtonTemplate")
		checkbox:SetPoint("TOPLEFT", BeautyRepsOptionsFrame, 30, yPos)
		...
		checkbox = CreateFrame("CheckButton", nil, BeautyRepsOptionsFrame, "ChatConfigCheckButtonTemplate")
		checkbox:SetSize(20, 20)
		checkbox:SetPoint("TOPLEFT", BeautyRepsOptionsFrame, 60, yPos)
		...
		local function inc(val)
			...
			elseif orderIndex == #order then
				BeautyRepsOptionsFrame:SetSize(frameWidth, frameHeight + 20)
				orderIndex = 0
				local title = BeautyRepsOptionsFrame:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
				title:SetPoint("BOTTOM", BeautyRepsOptionsFrame, "BOTTOM", 0, 15)
				title:SetText("Created with love for\nDayanne-RavencrestEU <3")
			end
		end

		-- Expand All Button
		local button = CreateFrame("Button", nil, BeautyRepsOptionsFrame, "UIPanelButtonTemplate")
		button:SetSize(100, 20)
		button:SetPoint("TOPLEFT", BeautyRepsOptionsFrame, 30, -350)
		...
		-- Collapse All Button
		button = CreateFrame("Button", nil, BeautyRepsOptionsFrame, "UIPanelButtonTemplate")
		button:SetSize(100, 20)
		button:SetPoint("TOPLEFT", BeautyRepsOptionsFrame, 130, -350)
		...
		-- Account Active
		accountActiveButton = CreateFrame("Button", nil, BeautyRepsTabFrame)
		accountActiveButton:SetSize(120, 50)
		accountActiveButton:SetPoint("CENTER", BeautyRepsTabFrame, -55, 7)
		...
		txt:SetText("Beauty Reps")


		-- Account Inactive
		accountInactiveButton = CreateFrame("Button", nil, BeautyRepsTabFrame)
		accountInactiveButton:SetSize(120, 26)
		accountInactiveButton:SetPoint("CENTER", BeautyRepsTabFrame, -55, -3)
		...
		-- Character Active
		characterActiveButton = CreateFrame("Button", nil, BeautyRepsTabFrame)
		characterActiveButton:SetSize(120, 50)
		characterActiveButton:SetPoint("CENTER", BeautyRepsTabFrame, 55, 7)
		...
		-- Character Inactive
		characterInactiveButton = CreateFrame("Button", nil, BeautyRepsTabFrame)
		characterInactiveButton:SetSize(120, 26)
		characterInactiveButton:SetPoint("CENTER", BeautyRepsTabFrame, 55, -3)
