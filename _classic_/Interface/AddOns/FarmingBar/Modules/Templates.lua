...
function addon:LoadTemplate(templateType, barID, templateName)
    local template, count

    if templateType == "user" then
        template = self.db.global.templates[strupper(templateName)]
    ...
    for buttonID, objectiveTable in pairs(template) do
        if objectiveTable.type == "item" then
            self.bars[barID].buttons[buttonID]:SetObjectiveID(objectiveTable.type, objectiveTable.itemID)
        elseif objectiveTable.type == "currency" then
            self.bars[barID].buttons[buttonID]:SetObjectiveID(objectiveTable.type, objectiveTable.currencyID)
        elseif objectiveTable.type == "mixedItems" or objectiveTable.type == "shoppingList" then
            self.bars[barID].buttons[buttonID]:SetObjectiveID(objectiveTable.type, objectiveTable.items, {title = objectiveTable.title, icon = objectiveTable.icon, objective = objectiveTable.objective})
        end
    end

    -- If number of visible bars is less than the items in the template, resize the bar to show all items
    if self.bars[barID].db.visibleButtons < count then
...
function addon:SaveTemplate(barID, templateName, overwrite)
    ...
    if self.db.global.templates[templateName] and not overwrite then
        -- Confirm overwrite
        ...
        for k, v in pairs(self.bars[barID].buttons) do
            if v.objective then
                tinsert(self.db.global.templates[templateName], v.objective)
            end
        end
...
StaticPopupDialogs["FARMINGBAR_CONFIRM_OVERWRITE_TEMPLATE"] = {
    text = "Farming Bar: Template \"%s\" already exists. Do you want to overwrite this template?",
    button1 = L["Yes"],
    button2 = L["No"],
