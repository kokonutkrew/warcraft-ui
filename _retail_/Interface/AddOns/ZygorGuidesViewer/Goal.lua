	...
	parse = function(self,params)
		...
		if self.targets then
			...
			for i,target in ipairs(self.targets) do
				if target[1] then
					local mult = {strsplit(" ",target[1])}
					for i,v in pairs(mult) do
						if v~="" then commons[v] = (commons[v] or 0) + 1 end
					end
				end
			end
			local maxcount,bestword = 0,""
			for i,v in pairs(commons) do
				if v>maxcount then
					maxcount = v
					bestword = i
				end
			end
			if maxcount == #self.targets then
				self.targetcommon = bestword.." enemies"
			end
		end
	end,
